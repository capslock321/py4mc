...
class MojangApi:
    ...
    ):  # Code seems ugly but works so... No touch.
        retrieved_profiles = []
        # nobody can have a valid uuid as a username
        if isinstance(profiles, str):
            route = Dispatch.API_BASE + f"/users/profiles/minecraft/{profiles}"
            response = Dispatch.do_request("GET", route)
            if not isinstance(response, dict):
                return None
            return self._get_profile_information(response.get("id"))
        profile_uuids = self._get_uuids(
            list(filter(lambda x: not is_valid_uuid(x), profiles))
        )
